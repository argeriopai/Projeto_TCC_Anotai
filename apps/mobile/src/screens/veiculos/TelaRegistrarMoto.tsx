...
import { cadastrarMotoApi, atualizarMotoApi, listarCarrosApi, listarMotosApi, Moto } from '../../services/api'
...
export function TelaRegistrarMoto({ navigation, route }: Props) {
  ...
  const temAlteracao = useCallback(() => {
    ...
    return !!marcaSel || !!modelo.trim() || !!placa.trim() ||
      !!cor.trim() || !!ano || !!freio || !!partida ||
      !!marcaCustom.trim()
  }, [edit, marcaSel, modelo, placa, cor, ano, freio,
      partida, marcaCustom])
  ...
  function handleSalvar() {
    requireAuth(async () => {
      ...
      try {
        ...
        const duplicado = motosExistentes.find(m =>
        ...
        if (duplicado) {
          Alert.alert(
            ...
            `Já existe uma moto registrada com a placa ${placa.toUpperCase()}.\n\nVerifique seus veículos cadastrados.`
          )
          setCarregando(false)
          return
        }
      ...
      try {
        ...
        const placaLimpa = placa.replace('-', '').toUpperCase()
        const duplicadoCarro = carrosExistentes.find(c =>
          c.placa.replace('-', '').toUpperCase() === placaLimpa
        )
        if (duplicadoCarro) {
          Alert.alert(
            '⚠️ Placa já cadastrada',
            `A placa ${placa.toUpperCase()} já está registrada em um carro.\n\nVerifique seus veículos cadastrados.`
          )
          setCarregando(false)
          return
        }
